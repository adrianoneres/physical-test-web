...
export default function InstitutionsNew() {
  ...
  const handleSave = useCallback(
    async (formData: FormProps) => {
      try {
        setLoading(true);
        await getApiClient().post('/institutions', formData);
        router.push({
          pathname: '/institutions',
          query: { message: `Polo ${formData.name} criado com sucesso` },
        });
      } catch (error) {
        handleError({
          error,
  ...
  return (
    <PrivateLayout title="Cadastro de Polo">
